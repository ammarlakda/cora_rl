rl_investment_planning
==============================
_Author: Crown Copyright_

Reinforcement Learning decision support for capital investment planning
...
```
cd rl_investment_planning
make create_environment
conda activate rl_investment_planning
```
...
```python
from rl-invest.data import Dataset
...
```python
from rl-invest import workflow
workflow.available_datasets(keys_only=True)
```

Note: sometimes datasets can be quite large. If you want to store your data externally, we recommend symlinking your data directory (that is `rl_investment_planning/data`) to somewhere with more room.
...
* Put any scripts or other code in the `rl-invest` module. We suggest you create a directory using the same initials you put in your notebook titles (e.g. `rl-invest/xyz`) You will be able to import it into your notebooks via `from rl-invest.xyz import ...`.
...
* `setup.py`
    * Turns contents of `rl-invest` into a
    pip-installable python module  (`pip install -e .`) so it can be
    imported in python code.
* `rl-invest`
    * Source code for use in this project.
    * `rl-invest/__init__.py`
        * Makes `rl-invest` a Python module.
    * `rl-invest/data`
